...
test_dataset_repeat <- read.csv("test_dataset/test_dataset_repeat.csv", header = FALSE)

test_dataset_repeat_matrix <- as.matrix(test_dataset_repeat)

cat("Original matrix dimensions:", dim(test_dataset_repeat_matrix), "\n") # Should be 10000 x 800

# --- EFFICIENT RESHAPING ---

# Step 2: Transpose the entire matrix from (10000 x 800) to (800 x 10000)
transposed_matrix <- t(test_dataset_repeat_matrix)
...
test_dataset_repeat_array <- aperm(temp_array, c(3, 2, 1))

# --- 3. DATA PREPARATION: Adjust all values in the arrays by adding 1 ---
cat("Adjusting all data points by adding 1...\n")
test_dataset_repeat_array <- test_dataset_repeat_array + 1
print(test_dataset_repeat_array[1, , ]) # Print the first slice to verify
...
datasets_to_process <- list(
  repeat_data = test_dataset_repeat_array
)
...
1.  `repeat_data_wide_rankings.csv`
