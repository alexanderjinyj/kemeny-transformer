...
Reads per-sample Kemeny distances from:
  - results/traditional_methods/{dataset_type}/ (H1, H2, KwikSort, MC)
  - results/decor/{dataset_type}/ (DECOR)
  - Transformer inference results (from scripts/inference.py output)

Produces plots like: results/figures/comparison_repeat_items_100_125_150.png
...
def load_traditional_distances(data_dir, dataset_type, n_voters, n_items):
    ...
    csv_path = os.path.join(
        data_dir, 'traditional_methods', dataset_type,
...
def load_decor_distances(data_dir, dataset_type, n_voters, n_items):
    """Load DECOR distances from results/decor CSV."""
    csv_path = os.path.join(
        data_dir, 'decor', dataset_type,
...
def load_transformer_distances(data_dir, dataset_type, n_voters, n_items, transformer_result_dir=None):
    ...
    # Default location: results/epoch_520_configs/{type}/
    search_paths.append(os.path.join(data_dir, 'epoch_520_configs', dataset_type, csv_name))

    # Other possible locations
    for subdir in ['results/epoch_520_configs',
                    'results/700k_various_voters_various_items_epoch_500']:
    ...
    for subdir in ['transformer_results/test_data', 'transformer_results/test_data_fine_tuning',
                    'transformer_results/fine_tuning']:
