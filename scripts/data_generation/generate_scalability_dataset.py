...
def main():
    for nb_candidates in [20,50,100,150,200]:
        ...
        print(f'test dataset repeat is all permutation={is_all_permutation}')
        os.makedirs("data/test/scalability", exist_ok=True)
        np.save(f"data/test/scalability/test_dataset_{nb_candidates}_random.npy",test_dataset_random)
        np.save(f"data/test/scalability/test_dataset_{nb_candidates}_repeat.npy",test_dataset_repeat)
        np.save(f"data/test/scalability/test_dataset_{nb_candidates}_jiggling.npy",test_dataset_jiggling)
