...
Unified result analysis script. Replaces:
  ...
  scalability_result_ananlysis.py

Usage examples:
  python scripts/run_result_analysis.py --data-type random
  python scripts/run_result_analysis.py --data-type repeat --fine-tuning
  python scripts/run_result_analysis.py --scalability --data-types random jiggling repeat --num-candidates 20 50 100 150
"""
import argparse
import os
...
def parse_args():
    ...
    parser.add_argument('--data-types', nargs='+', default=['random', 'jiggling', 'repeat'],
                        help='Data types for scalability analysis')
    parser.add_argument('--fine-tuning', action='store_true',
                        help='Use fine-tuning results')
    parser.add_argument('--scalability', action='store_true',
                        help='Run scalability analysis mode')
    parser.add_argument('--num-candidates', nargs='+', type=int, default=[20, 50, 100, 150],
                        help='Number of candidates for scalability analysis')
    parser.add_argument('--data-dir', default='data/test',
                        help='Base data directory (input datasets)')
    parser.add_argument('--results-dir', default='results',
                        help='Base results directory (gurobi/transformer outputs, analysis)')
...
def run_standard_analysis(data_type, fine_tuning, data_dir, results_dir):
    """Run analysis for a single data type (standard or fine-tuning)."""
    dt = data_type

    # Load Gurobi results
    gurobi_data = np.load(
        f'{results_dir}/gurobi/test_dataset_{dt}_kemeny_optimal_ranking_gurobi.npy',
        allow_pickle=True
    )
    optimal_distances = np.load(
        f'{results_dir}/gurobi/test_dataset_{dt}_kemeny_optimal_ranking_distance_gurobi.npy',
        allow_pickle=True
    )
    ...
    if fine_tuning:
        transformer_path = f'{results_dir}/transformer/test_data_fine_tuning/test_dataset_{dt}_transformer.npy'
    else:
        transformer_path = f'{results_dir}/transformer/test_data/test_dataset_{dt}_transformer.npy'
    ...
    # Save results
    if fine_tuning:
        output_dir = f'{results_dir}/analysis_data/fine_tuning'
        output_file = f'{output_dir}/analysis_data_2000_{dt}_fine_tuning.npy'
    else:
        output_dir = f'{results_dir}/analysis_data'
...
def run_scalability_analysis(data_types, num_candidates_list, data_dir, results_dir):
    """Run scalability analysis across multiple data types and candidate counts."""
    for data_type in data_types:
        for nb_candidates in num_candidates_list:
            ...
            result = {}
            gurobi_data = np.load(
                f"{results_dir}/gurobi/scalability/test_dataset_{nb_candidates}_{data_type}_kemeny_optimal_ranking_gurobi.npy",
                allow_pickle=True
            )
            ...
            base_rankings = np.load(f"{data_dir}/scalability/test_dataset_{nb_candidates}_{data_type}.npy")[0:gurobi_size]

            transformer_data = np.load(
                f"{results_dir}/transformer/fine_tuning/test_dataset_{nb_candidates}_{data_type}_transformer.npy",
                allow_pickle=True
            )
            ...
            output_dir = f"{results_dir}/analysis_data/fine_tuning/scalability"
            os.makedirs(output_dir, exist_ok=True)
...
def main():
    ...
    if args.scalability:
        run_scalability_analysis(args.data_types, args.num_candidates, args.data_dir, args.results_dir)
    else:
        if args.data_type is None:
            print("Error: --data-type is required for standard/fine-tuning mode")
            return
        run_standard_analysis(args.data_type, args.fine_tuning, args.data_dir, args.results_dir)
