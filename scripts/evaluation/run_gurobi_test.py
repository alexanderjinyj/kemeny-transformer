...
Unified Gurobi test script. Replaces:
  ...
  gurobi_test_100_25.py, gurobi_test_75_50.py, gurobi_scalability_test.py
...
Usage examples:
  ...
  # Scalability mode (replaces gurobi_scalability_test.py):
  python scripts/run_gurobi_test.py --mode scalability --data-types repeat --num-candidates 150 --max-samples 25
"""
import argparse
import os
import os.path
...
def parse_args():
    ...
    parser.add_argument('--mode', choices=['standard', 'ablation', 'scalability'], required=True,
                        help='Test mode: standard (100 items), ablation (variable items), scalability (scalability)')
    ...
    parser.add_argument('--max-samples', type=int, default=None,
                        help='Limit number of samples (e.g., 25 for scalability)')
    parser.add_argument('--data-dir', default='data/test',
                        help='Base data directory')
    parser.add_argument('--output-dir', default='results/gurobi',
                        help='Base output directory for Gurobi results')
    ...
    for num in args.num_candidates:
        for data_type in args.data_types:
            ...
            if args.mode == 'standard':
                ...
                data_file = f'{args.data_dir}/ablation/test_dataset_{data_type}_{num}.npy'
                ...
                data_file = f'{args.data_dir}/scalability/test_dataset_{num}_{data_type}.npy'
                output_dir = f'{args.output_dir}/scalability'
                output_path = f'{output_dir}/test_dataset_{num}_{data_type}_kemeny_optimal_ranking_gurobi.npy'
                distance_path = None  # Scalability saves everything in one file
