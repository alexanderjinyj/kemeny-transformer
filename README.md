...
```
kemeny-transformer/
    kemeny_transformer/              # Python package (the library)
        model/
        ...
        baselines/
            ...
            spearman.py              # Spearman footrule consensus
    ...
    scripts/
        train.py                     # Training (single/multi-GPU with DDP)
        inference.py                 # Inference with greedy/beam search
        data_generation/
            generate_test_dataset.py        # Ablation test datasets (25-100 items)
            generate_validation_dataset.py  # Validation datasets
            generate_scalability_dataset.py # Scalability datasets (20-200 items)
        evaluation/
            run_gurobi_test.py       # Compute exact Gurobi solutions
            run_result_analysis.py   # Analyze transformer vs Gurobi results
            compute_borda_footrule.py# Score Borda / footrule baselines
            compute_decor_distances.py # Score DECOR (R) rankings
            run_input_norm_ablation.py # Input-normalization ablation
            test_various_input.py    # Test on variable-size inputs
        plotting/
            plot_results.py          # Gap-from-best comparison plots
            plot_running_time.py     # Scalability running-time plots
            plot_input_norm_ablation.py # Ablation plots

    r_scripts/
        decor_test.R                 # DECOR baseline on one dataset (parameterized)
        decor_all_datasets_parallel.R# DECOR on all datasets in parallel

    configs/                         # Training configuration files (JSON)
    notebooks/                       # Jupyter analysis notebooks

    data/                            # INPUT datasets
        test/{random,repeat,jiggling}/   # Pre-generated test sets (.npy/.csv)
        test/ablation/               # Variable-item-count test sets (generated)
        test/scalability/            # Scalability test sets (generated)
        validation/                  # Validation sets (+ Gurobi optima)
        nation/                      # Nation-ranking case study source data

    results/                         # EXPERIMENT results
        synthetic/                   # Transformer results on 8x100 test sets
        generalization/              # Generalization experiments
        traditional_methods/         # H1/H2/KwikSort/Markov-chain baselines
        decor/                       # DECOR baseline rankings + distances
        borda_footrule/              # Borda / footrule baseline scores
        gurobi/                      # Gurobi optimal solutions (generated)
        epoch_520_configs/           # Per-config transformer results @ epoch 520
        input_norm_ablation/         # Input-normalization ablation results
        preflib_university/          # PrefLib university-ranking case study
        nation/                      # Nation-ranking case study results
        figures/                     # All generated plots

    outputs/                         # Training artifacts (gitignored)
        checkpoints/                 # Model checkpoints (.pkl)
        logs/validation/             # Per-epoch validation logs
        logs/rollout/                # Actor-vs-baseline rollout logs

    archive/                         # Superseded scripts & duplicated data kept
                                     # for reference (not used by anything)
```
...
```bash
# Generate ablation test datasets (25, 50, 75, 100 candidates, 8 voters)
#   -> data/test/ablation/
PYTHONPATH=. python scripts/data_generation/generate_test_dataset.py

# Generate scalability test datasets (20, 50, 100, 150, 200 candidates)
#   -> data/test/scalability/
PYTHONPATH=. python scripts/data_generation/generate_scalability_dataset.py

# Generate validation datasets (6 and 10 voters, 100 candidates)
#   -> data/validation/
PYTHONPATH=. python scripts/data_generation/generate_validation_dataset.py
```

Each script generates datasets for all three perturbation types (random, repeat, jiggling).
...
**Single GPU:**
```bash
PYTHONPATH=. python scripts/train.py configs/args_8_voter_100_items.conf
```

**Multi-GPU (DDP):**
```bash
PYTHONPATH=. torchrun --nproc_per_node=2 scripts/train.py configs/args_8_voter_100_items.conf
```
...
- **Checkpoints**: `outputs/checkpoints/.../*.pkl` (model weights, optimizer state, epoch)
- **Validation logs**: `outputs/logs/validation/.../validation_*.txt` (per-epoch gap metrics)
- **Rollout logs**: `outputs/logs/rollout/.../roll_out_*.txt` (actor vs baseline performance)
...
PYTHONPATH=. python scripts/inference.py \
    ...
    --checkpoint outputs/checkpoints/.../checkpoint_epoch_500.pkl \
    --dataset data/test/test_dataset_random.npy \
    --output-dir results/transformer/test_data \
...
Gurobi solves the Kemeny problem exactly via integer linear programming (ILP). It's the ground truth but very slow for large inputs. Results are written under `results/gurobi/`.

```bash
# Standard test (100 candidates):
PYTHONPATH=. python scripts/evaluation/run_gurobi_test.py --mode standard --data-types random repeat jiggling

# Ablation (variable candidates, reads data/test/ablation/):
PYTHONPATH=. python scripts/evaluation/run_gurobi_test.py --mode ablation --data-types random --num-candidates 25 50

# Scalability test (reads data/test/scalability/):
PYTHONPATH=. python scripts/evaluation/run_gurobi_test.py --mode scalability --data-types repeat --num-candidates 150 --max-samples 25
```
...
```bash
# Standard analysis:
PYTHONPATH=. python scripts/evaluation/run_result_analysis.py --data-type random

# Fine-tuning results:
PYTHONPATH=. python scripts/evaluation/run_result_analysis.py --data-type random --fine-tuning

# Scalability analysis:
PYTHONPATH=. python scripts/evaluation/run_result_analysis.py --scalability --data-types random repeat jiggling --num-candidates 20 50 100 150
```

### Other Baselines

Heuristic baselines included for comparison:
- **Borda** (`kemeny_transformer.baselines.borda`): Borda count consensus
- **Spearman footrule** (`kemeny_transformer.baselines.spearman`): Footrule-optimal consensus
...
- **DECOR** (`r_scripts/`): Differential evolution consensus (ConsRank R package)

```bash
# Score Borda + footrule on the test sets -> results/borda_footrule/
...
# Run DECOR (R) on one dataset -> results/decor/
Rscript r_scripts/decor_test.R random 2000 8 100
Rscript r_scripts/decor_all_datasets_parallel.R

# Score the DECOR rankings with Kemeny distance
...
```bash
# Gap-from-best comparison across all methods -> results/figures/
PYTHONPATH=. python scripts/plotting/plot_results.py

# Scalability running-time plots
PYTHONPATH=. python scripts/plotting/plot_running_time.py

# Input-normalization ablation plots
PYTHONPATH=. python scripts/plotting/plot_input_norm_ablation.py
```
...
PYTHONPATH=. torchrun --nproc_per_node=2 scripts/train.py configs/args_700k_linear_various_voter_various_items_mix_batch.conf
...
PYTHONPATH=. python scripts/inference.py \
    ...
    --checkpoint outputs/checkpoints/.../checkpoint_epoch_650.pkl \
    --dataset data/test/ablation/test_dataset_random_100.npy \
    --output-dir results/transformer/test_data \
    --output-name test_dataset_random

# 6. Compute Gurobi baselines
PYTHONPATH=. python scripts/evaluation/run_gurobi_test.py --mode standard --data-types random
