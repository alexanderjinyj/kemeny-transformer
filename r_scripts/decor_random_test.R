...
test_dataset_random <- read.csv("test_dataset/test_dataset_random.csv", header = FALSE)

test_dataset_random_matrix <- as.matrix(test_dataset_random)

cat("Original matrix dimensions:", dim(test_dataset_random_matrix), "\n") # Should be 10000 x 800

# --- EFFICIENT RESHAPING ---

# Step 2: Transpose the entire matrix from (10000 x 800) to (800 x 10000)
transposed_matrix <- t(test_dataset_random_matrix)
...
# So, (100, 8, 10000)
temp_array <- array(transposed_matrix, dim = c(100, 8, 2000))
...
test_dataset_random_array <- aperm(temp_array, c(3, 2, 1))

# --- 3. DATA PREPARATION: Adjust all values in the arrays by adding 1 ---
cat("Adjusting all data points by adding 1...\n")
test_dataset_random_array <- test_dataset_random_array + 1
print(test_dataset_random_array[1, , ]) # Print the first slice to verify
...
datasets_to_process <- list(
  random_data = test_dataset_random_array
)
...
1.  `random_data_wide_rankings.csv`
